...
INLINE void update_accumulator(const Position *pos, const Color c)
{
  ...
  while (st->accumulator.state[c] == ACC_EMPTY)
  {
    ...
    if (dp->pc[0] == make_piece(c, KING) || (gain -= st->added[c].size + st->removed[c].size + 1) < 0)
      break;
    st--;
  }
...
INLINE int transform(const Position *pos, clipped_t *output, uint16_t *nnz_indices, int32_t psqtBucket, int32_t *psqt_val)
{
  ...
  *psqt_val = ((*accumulation_psqt)[perspectives[0]][psqtBucket] - (*accumulation_psqt)[perspectives[1]][psqtBucket]) / 2;
...
void nnue_init(void)
{
  ...
  useNNUE = strcmp(s, "classical") == 0 ? EVAL_CLASSICAL
            : strcmp(s, "pure") == 0    ? EVAL_PURE
                                        : EVAL_HYBRID;
#endif

  const char *evalFile = "large_spheres.nnue";
