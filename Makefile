...
PGOBENCH = ./$(EXE) bench 16 1 16 default depth nnue
...
# pretends to be GCC on some systems.
	...
	ifeq ($(gccisclang),)
		CFLAGS += -flto
#		LDFLAGS += $(CFLAGS) -flto=jobserver jobserver removed NS /8/29/2022
		LDFLAGS += $(CFLAGS)
		ifneq ($(findstring MINGW,$(KERNEL)),)
			LDFLAGS += -save-temps
		else ifneq ($(findstring MSYS,$(KERNEL)),)
...
help:
	@echo ""
	@echo "To compile Large Spheres, type: "
	@echo ""
...
profileclean:
	...
	@rm -f large_spheres.profdata *.profraw
...
clang-profile-use:
	$(XCRUN) llvm-profdata merge -output=large_spheres.profdata *.profraw
	$(MAKE) ARCH=$(ARCH) COMP=$(COMP) \
	EXTRACFLAGS='-fprofile-instr-use=large_spheres.profdata' \
